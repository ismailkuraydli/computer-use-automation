 ...
 * (the cross-tenant reuse case). Keystone is the base the artifacts are
 * recorded on; Summit renames labels, moves the member page and adds its
 * own security reminder.
 */
export type MockVariant = "keystone" | "summit";
...
interface VariantConfig {
  ...
  /** Express route of the member detail page, and the link to it. */
  detailRoute: string;
  detailHref: (id: string) => string;
  securityReminder: boolean;
}

const VARIANTS: Record<MockVariant, VariantConfig> = {
  keystone: {
    brand: "Keystone CU",
    memberIdLabel: "Member ID",
    balanceColumn: "Balance",
    detailHeading: "Member Detail",
    detailRoute: "/detail",
    detailHref: (id) => `/detail?id=${id}`,
    securityReminder: false,
  },
  summit: {
    brand: "Summit FCU",
    memberIdLabel: "Account Holder #",
    balanceColumn: "Available Balance",
    detailHeading: "Account Holder Detail",
    detailRoute: "/members/:id",
    detailHref: (id) => `/members/${id}`,
...
function overlay(id: string, title: string, text: string, button: string): string {
  return `
  <div id="${id}" style="position:fixed;top:0;left:0;right:0;bottom:0;background:rgba(0,0,0,0.5);z-index:1000">
  <table style="background:#fff;margin:120px auto;border:2px solid #333" cellpadding="8">
  <tr><td class="cls5"><b>${title}</b></td></tr>
  <tr><td>${text}</td></tr>
  <tr><td><input type="button" value="${button}" onclick="document.getElementById('${id}').remove()"></td></tr>
  </table></div>`;
}

const NOTICE_OVERLAY = overlay(
  "noticeOverlay",
  "System Notice",
  "Scheduled maintenance tonight 11pm-2am. Some functions may be unavailable.",
  "Acknowledge"
);

const SECURITY_REMINDER = overlay(
  "securityReminder",
  "Security Reminder",
  "Never share your credentials. Verify the member before disclosing account data.",
  "Dismiss"
);
...
export function createMockApp(opts: MockAppOptions = {}): MockApp {
  const v = VARIANTS[opts.variant ?? "keystone"];
  ...
  app.get("/search", (req, res) => {
    ...
    let html = `<html><head><title>${v.brand} - Member Search</title>
      ...
      <tr><td>${v.memberIdLabel}:</td><td><input type="text" name="q" aria-label="${v.memberIdLabel}" value="${escapeHtml(query)}" size="20"></td>
    ...
    if (query) {
      ...
      if (member) {
        html += `<tr><td class="cls2">
          ...
          <tr class="cls4"><th>${v.memberIdLabel}</th><th>Name</th><th>SSN</th><th>Accounts</th></tr>
          <tr><td><a href="${v.detailHref(escapeHtml(query))}">${escapeHtml(query)}</a></td>
          <td>${member.name}</td><td>${member.ssn}</td>
          <td>${member.accounts.length}</td></tr>
          </table></td></tr>`;
      } else {
        html += `<tr><td class="cls2"><table border="0" cellpadding="4">
          <tr><td class="cls5"><b>No records found for ${v.memberIdLabel}: ${escapeHtml(query)}</b></td></tr>
          </table></td></tr>`;
      }
    }

    html += `</table>${v.securityReminder ? SECURITY_REMINDER : ""}</body></html>`;
    res.type("html").send(html);
  });

  app.get(v.detailRoute, (req, res) => {
    ...
    res.type("html").send(`<html><head><title>${v.brand} - Member Detail</title>
      <link rel="stylesheet" href="/style.css"></head><body>
      <table class="cls1" border="0" cellpadding="4" cellspacing="0" width="100%">
      <tr><td class="cls2"><h1>${v.detailHeading} - ${member.name}</h1></td></tr>
      <tr><td class="cls2">
      <table border="1" cellpadding="4" cellspacing="0" class="cls3" width="80%">
      <tr class="cls4"><th>${v.memberIdLabel}</th><th>SSN</th><th>DOB</th></tr>
      ...
      <tr><td class="cls2">
      <table border="1" cellpadding="4" cellspacing="0" class="cls3" width="80%">
      <tr class="cls4"><th>Type</th><th>Account Number</th><th>${v.balanceColumn}</th><th>Action</th></tr>
      ${accountsRows}
      </table></td></tr>
      <tr><td class="cls2">&nbsp;</td></tr>
  ...
  app.get("/account-action", (req, res) => {
    ...
    res.type("html").send(`<html><head><title>${v.brand} - Account</title>
      <link rel="stylesheet" href="/style.css"></head><body>
      <table class="cls1" border="0" cellpadding="4" cellspacing="0" width="100%">
      ...
      <tr><td class="cls2"><a href="${v.detailHref(id)}">Back to Member Detail</a></td></tr>
      </table></body></html>`);
  });
  ...
  app.get("/new-account", (req, res) => {
    ...
    if (member.restricted) {
      res.status(403).type("html").send(`<html><head><title>${v.brand} - Access Denied</title>
        <link rel="stylesheet" href="/style.css"></head><body>
    ...
    res.type("html").send(`<html><head><title>${v.brand} - New Sub-Account</title>
      <link rel="stylesheet" href="/style.css"></head><body>
      <table class="cls1" border="0" cellpadding="4" cellspacing="0" width="100%">
  ...
  app.post("/new-account-confirm", (req, res) => {
    ...
    if (!deposit || isNaN(Number(deposit)) || Number(deposit) <= 0) {
      res.type("html").send(`<html><head><title>${v.brand} - Error</title>
        <link rel="stylesheet" href="/style.css"></head><body>
        <table class="cls1" border="0" cellpadding="4" cellspacing="0" width="100%">
    ...
    res.type("html").send(`<html><head><title>${v.brand} - Confirmation</title>
      <link rel="stylesheet" href="/style.css"></head><body>
      <table class="cls1" border="0" cellpadding="4" cellspacing="0" width="100%">
      ...
      <tr><td class="cls2">&nbsp;</td></tr>
      <tr><td class="cls2"><a href="${v.detailHref(escapeHtml(memberId))}">Back to Member Detail</a></td></tr>
      </table></body></html>`);
  });

  // Main page with iframes (hostile: frames-based layout)
  app.get("/", (_req, res) => {
    res.type("html").send(`<html><head><title>${v.brand} - Member Servicing Portal</title>
      <link rel="stylesheet" href="/style.css"></head><body>
